%% FUNCTION JAIL
function [tauB uwheel controlpath]=controller(t,x,J,gyro) %using zac's paper
    ...
    tolp=0.05; %error tolerance for pointing
    uwheel=[0;0;0];
    [Jval ind]=max(max(J));
    Jmax=[0;0;1];
...
function [xnew,tauB,controlpath]=rk4(x,h,J,t,gyro)
    [tauB,uwheel,controlpath]=controller(t,x,J,gyro);
...
gyro=zeros(3,n);
bias=zeros(3,n);
bias(:,1)=0.0*randn(3,1); %initial bias
M=0.001*eye(3,3)+eye(3,3); %mounting error
M=expm(hat(0.01*randn(3,1)))*M*(expm(hat(0.01*randn(3,1))))';
V=0.00000001*eye(6,6);
for k=1:n-1
    [xmat(:,k+1) tauBmat(:,k+1) controlpathvec(:,k+1)]=rk4(xmat(:,k),h,J,tspan(k),gyro(:,k));
    gyro(:,k+1)=M*xmat(5:7,k)+bias(:,k)+sqrt(V(4:6,4:6))*randn(3,1);
    bias(:,k+1)=bias(:,k)+0.00001*randn(3,1); %random walk
end
